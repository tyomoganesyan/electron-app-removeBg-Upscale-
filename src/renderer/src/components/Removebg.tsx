...
export const Removebg = () => {
  ...
  useEffect(() => {
    const loadState = async () => {
      try {
        const response = await window.api.loadState(false)
        if (response) {
          //   setFiles(response.files || [])
          //   setHandledFiles(response.handledFiles || [])
          setOutputType(response.outputType || 'cutout')
          setFormat(response.format || 'JPG')
          setColor(response.color || '#FFFFFF')
          setSelectedOption(
  ...
  useEffect(() => {
    return () => {
      ...
      if (
        ...
        window.api.saveState(currentState, false)
      }
      prevStateRef.current = currentState
    }
  }, [files, handledFiles, outputType, format, color, selectedOption])
  ...
  return (
    <>
      ...
      <div className={styles.end}>
        <button className={styles.start} onClick={processFiles}>
          Process
        </button>
      </div>
    </>
