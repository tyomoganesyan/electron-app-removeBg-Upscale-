...
export const Upscale = () => {
  ...
  useEffect(() => {
    const loadState = async () => {
      try {
        const response = await window.api.loadState(true)
        if (response) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (
        ...
        window.api.saveState(currentState, true)
      }
  ...
  async function processFiles(): Promise<IResponse[]> {
    ...
    for (const file of files) {
      ...
      formData.append(
        'upscale_factor',
        String(upscaleFactor[index]) === 'undefined' ? '2' : String(upscaleFactor[index])
      )
      index++
  ...
  return (
    <>
      ...
      <Box
        sx={{ display: 'flex', gap: 2, marginLeft: 5, width: 'auto', height: 45, marginBottom: 2 }}
      >
